...
/** \brief  Lock G2 bus and enable CPU access

    This function will ensure that the G2 bus can safely be accessed by the CPU
    through the g2_read_*() / g2_write_*() functions.
    As this will pause any ongoing DMA transfer and prevent new ones from
    starting, as soon as you are done accessing the G2 bus you should lock it
    again by calling g2_unlock() with the context returned by this function.

    \return                 The G2 context to be used with g2_unlock().
*/
g2_ctx_t g2_lock(void);

/** \brief  Disable CPU access and unlock G2 bus

    This function will restore the normal function of the G2 bus. After this
    call, any CPU access to the G2 bus is unsafe.

    \param  ctx             The G2 context returned by g2_lock().
*/
