...
static struct aica_voice aica_voices[62];

void s3m_play(struct s3m_header *header, size_t len)
{
    buffer = header;

    aica_printf("MikMod init...\n");
    MikMod_Init("");

    MikMod_RegisterAllLoaders();

    aica_printf("Loading module...\n");
    module = Player_LoadMem(buffer, len, NB_CHANNELS_MAX, 0);
    if (!module) {
        aica_printf("Unable to initialize module: %d\n", MikMod_errno);
        return;
    }

    aica_printf("Module loaded. Starting playback...\n");
    Player_Start(module);

    aica_printf("Playback started!\n");
}
...
static void aica_run_mikmod(void)
{
    ...
    while (!stopped) {
        ...
        ms = 2500 / (volatile UWORD)module->bpm;
        task_sleep(ms_to_ticks(ms));
    }
}
                            ...
                            UWORD flags)
{
    ...
    aica_voices[voice] = (struct aica_voice){
        .freq = mmsample->speed,
        .vol = mmsample->volume,
        .pan = mmsample->panning,
        .playing = 1,
    };
